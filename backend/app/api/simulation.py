...
def delete_simulation(simulation_id):
    ...
    try:
        ...
        manager = SimulationManager()
        manager.delete_simulation(simulation_id, soft_delete=soft_delete, reason=reason)
        return jsonify({"success": True, "message": "Simulation deleted successfully"})
    except ValueError as e:
...
def restore_simulation(simulation_id):
    """恢复已删除的模拟"""
    try:
        manager = SimulationManager()
        manager.restore_simulation(simulation_id)
        return jsonify({"success": True, "message": "Simulation restored successfully"})
    except ValueError as e:
        return jsonify({"success": False, "error": str(e)}), 404
    except Exception as e:
