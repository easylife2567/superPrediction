    ...
    try:
        ...
        if existing_report and existing_report.status == ReportStatus.COMPLETED:
            return jsonify({
                "success": True,
                "data": {
                    "simulation_id": existing_report.simulation_id,
                    "report_id": existing_report.report_id,
                    "status": "completed",
                    "progress": 100,
                    "message": "报告已生成",
        ...
        # 如果没有提供 task_id 但提供了 report_id / simulation_id，从内存任务池中寻找
        if not task_id:
            all_tasks = task_manager.list_tasks(task_type="report_generate")
            for t in all_tasks:
                meta = t.get("metadata", {})
                if (simulation_id and meta.get("simulation_id") == simulation_id) or \
                   (report_id and meta.get("report_id") == report_id):
        ...
        if not task_id:
            if existing_report:
                return jsonify({
                    "success": True,
                    "data": {
                        "status": existing_report.status.value if existing_report.status else "failed",
                        "error": "该任务未在后台运行(可能已中断)",
                        "progress": 0
                    }
                })
            return jsonify({
                "success": False,
                "error": "缺少有效的 task_id、report_id 或 simulation_id 用于反查任务"
            }), 400
