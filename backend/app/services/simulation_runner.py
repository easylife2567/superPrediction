...
class SimulationRunner:
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        ...
        if state:
            # 僵尸进程检测: 如果状态是运行中，但在内存中找不到对应的进程对象，说明服务重启了
            if state.runner_status in [RunnerStatus.RUNNING, RunnerStatus.STARTING]:
                if simulation_id not in cls._processes:
                    logger.warning(f"检测到僵尸模拟状态: {simulation_id}, status={state.runner_status}, 但进程丢失。重置为 FAILED。")
                    state.runner_status = RunnerStatus.FAILED
                    state.error = "服务异常重启，模拟进程已中断。系统将自动重新开始。"
