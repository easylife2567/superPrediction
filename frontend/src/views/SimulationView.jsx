...
const SimulationView = () => {
    ...
    const getLeftStyle = () => {
        ...
        if (viewMode === 'workbench') return { width: '0%', opacity: 0, transform: 'translateX(-20px)' };
        return { width: '50%', opacity: 1, transform: 'translateX(0)' };
    };

    const getRightStyle = () => {
        if (viewMode === 'workbench') return { width: '100%', opacity: 1, transform: 'translateX(0)' };
            ...
            <header className="app-header">
                ...
                <div className="header-center">
                    <div className="view-switcher">
                        {['graph', 'split', 'workbench'].map(mode => (
                            <button key={mode} className={`switch-btn ${viewMode === mode ? 'active' : ''}`} onClick={() => setViewMode(mode)}>
                ...
                <div className="header-right">
                    ...
                    <span className={`status-indicator ${currentStatus}`}>
                        ...
                        {currentStatus === 'completed' ? 'Ready' : currentStatus === 'error' ? 'Error' : 'Preparing'}
                    </span>
                </div>
            </header>
